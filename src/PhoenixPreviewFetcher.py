...
from model.Preview import Preview, PreviewLine, PreviewDataHolder
    ...
    def load(self, data):
        ...
        for result in results:
            ...
            preview_headers.append(Preview(str(user_id), title, details, start_date,
                                                end_date, str(is_active), business_key).generate_record())
            ...
            for stop in stops:
                ...
                preview_lines.append(PreviewLine(str(user_id), country, region, county, town, postcode,
                                                      details, start_date, end_date, business_key).generate_record())

        return PreviewDataHolder(preview_headers, preview_lines)
        ...
        if len(user_profiles) > 0:
            ...
            for user_profile in user_profiles:
                ...
                if user_id is not None and user_id not in existing_user_id_list:
                    ...
                    response = requests.get(f"{self.propertyManager.getApiProviderUrl()}/api/Content/Tour/{user_id}?v=2", headers=headers)

                    # add a pause here to control the rate of API invocation
                    time.sleep(1)
                    if response.status_code != 200:
                        print(f"An error has occurred when fetching preview data for {user_id}. Status code: {response.status_code}")
                    else:
                        ...
                        if len(preview_data_holder.get_preview_header()) > 0:
                            ...
                            print("===================")
                            for p in preview_data_holder.get_preview_header():
                                print(p)
                            print("===================")
