...
PHOENIX_HOME="${CURRENT_LOCATION}/${APPLICATION_NAME}/src"
VERSION="3.0.14"
IS_UPDATE="false"
...
if [ $1 = "--help" ]; then
  echo "[INFO] Usage:"
  echo "[INFO] sh install-src.sh --clean <version_number>"
  echo "[INFO] sh install-src.sh --update <version_number>"
  echo "[INFO] sh install-src.sh --env-setup"
  echo "[INFO] sh install-src.sh --help"
  exit
fi
...
if [ $1 = "--update" ]; then
  ...
  if [ ! -f "${PHOENIX_HOME}/runp.sh" ]; then
...
mkdir -p "$TEMP_DIR"
# download file
curl -L -o ${TEMP_DIR}/phoenix.zip https://github.com/EngineX-GB/${APPLICATION_NAME}/releases/download/${VERSION}/${APPLICATION_NAME}_${VERSION}.zip
# unzip
if [ $? -eq 0 ]; then
    unzip "${TEMP_DIR}/phoenix.zip" -d "${INSTALL_DIR}"
